# This migration comes from refinery_entertainments (originally 1)
class CreateEntertainmentsEntertainments < ActiveRecord::Migration

  def up
    create_table :refinery_entertainments do |t|
      t.string :headline_1
  ...
  def down
    if defined?(::Refinery::UserPlugin)
      ::Refinery::UserPlugin.destroy_all({:name => "refinerycms-entertainments"})
    end

    if defined?(::Refinery::Page)
      ::Refinery::Page.delete_all({:link_url => "/entertainments/entertainments"})
    end

    drop_table :refinery_entertainments
